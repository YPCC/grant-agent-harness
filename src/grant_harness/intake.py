"""Office of Research Aid intake fill + human override (standalone)."""
...
INTAKE_CATALOG = PACKS_DIR / "ora_intake.yaml"
...
def load_intake_catalog(path: Path | None = None) -> dict[str, Any]:
    ...
    data.setdefault("not", "NIH_ASSIST")
    return data
...
def _normalize(raw: str) -> str:
    ...
    if s in NO:
        return "no"
    if s in NA or s in {"not_applicable"}:
...
def _detect_value(item: dict, hay: str, filename: str, char_count: int) -> tuple[str, str]:
    ...
    needles = [str(n).lower() for n in (item.get("detect") or []) if n]
    ...
    if item["id"] == "RS_PAGE_LIMIT":
        return "not_applicable", "Aims-only draft assumed unless Research Strategy is present"
...
def fill_intake(
    ...
    return {
        ...
        "not": catalog.get("not", "NIH_ASSIST"),
        "items": rows,
        ...
        "summary": (
            ...
            + (
                "Ready to submit to Office of Research Aid."
                if can_submit
                else f"Still needed: {', '.join(r['id'] for r in incomplete)}."
            )
        ),
    }
