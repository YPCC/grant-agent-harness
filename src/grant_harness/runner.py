"""Path-agnostic harness runner with scripted HITL."""
...
from grant_harness.graders import grade_run
from grant_harness.intake import apply_overrides, fill_intake
from grant_harness.knowledge import resolve_packs
from grant_harness.packaging import create_office_package
...
def run_pipeline(
    ...
    *,
    ...
    """Execute knowledge → review → checklist → intake → HITL → optional ORA package."""
    ...
    for step in script:
        ...
        if step == "revise":
            # deterministic pass: re-score same text (writer is out of band in v0)
            review = review_text(text, mechanism=mechanism)
            checklist = evaluate_checklist(text)
        elif step == "waive":
    ...
    package = None
    package_ready = bool(checklist["can_freeze"] and hitl["decision"] == "approve" and intake["can_submit_to_office"])
...
def run_case_id(case_id: str, **kwargs: Any) -> dict[str, Any]:
    from grant_harness.cases import load_case
